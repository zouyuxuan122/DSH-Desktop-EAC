...
function createGuard(opts) {
  ...
  function snapshot(reason) {
    try {
      ...
      // 同一毫秒内多次快照（启动链会连续拍几张）：时间戳会撞名互相覆盖，
      // 追加补零序号保证唯一且字典序仍与时间序一致。
      let id = stamp;
      let dest = path.join(rollbacksDir(), id);
      for (let n = 1; fs.existsSync(dest); n += 1) {
        id = `${stamp}-${String(n).padStart(3, '0')}`;
        dest = path.join(rollbacksDir(), id);
      }
      ...
      const meta = {
        id, reason: String(reason || 'manual'), at: new Date().toISOString(),
        files, pluginRows: rows,
      };
      writeJson(path.join(dest, 'meta.json'), meta);
      pruneSnapshots();
      log('guard', `已创建快照 ${id}（${reason}，${files.length} 个文件，${rows.length} 个插件行）`);
      return meta;
    } catch (err) {
