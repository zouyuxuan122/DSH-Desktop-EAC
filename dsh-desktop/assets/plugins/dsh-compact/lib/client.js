      ...
      return h('li', { className: 'dshc-card', 'data-testid': 'dsh-compact-settings' },
        h('div', { className: 'dshc-head' },
          h('div', null,
            ...
            h('p', { className: 'dshc-desc' }, '在模型请求前按真实 Token 压力压缩；溢出恢复会先复检实测上下文，防供应商误报误压；瞬态 400 自动重试一次。'),
          ),
        ...
        h(Field, { label: '溢出自动恢复', hint: '收到 CONTEXT_WINDOW_EXCEEDED 且实测 tokens 接近上下文窗口时才压缩并重试；远低于窗口的误报会原样透传 400。' },
          h('input', {
            type: 'checkbox',
        ...
        h(Field, { label: '400 瞬态错误自动重试', hint: '非溢出的 400（如免费服务商偶发）且本会话此前已有成功回答时，自动原样重试一次，免去「无故 400、继续说一句才好」；彻底失败才提示。' },
          h('input', {
            type: 'checkbox',
            checked: value.retryTransientBadRequest !== false,
            disabled: !writable,
            onChange: (event) => set('retryTransientBadRequest', event.target.checked),
          }),
        ),
