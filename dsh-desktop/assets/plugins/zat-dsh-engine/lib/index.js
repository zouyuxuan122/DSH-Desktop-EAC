...
var kinds_default = {
	...
	"csyangwen/dsh-memory-evolve": "nonplugin",
	...
	"whiteguo233/dsh-openbiliclaw": "nonplugin",
	...
	"lhh010/dsh-ui-progress": "nonplugin",
	...
	"lhh010/dsh-paste-input": "nonplugin",
	...
	"opensetk/dsh-xiaohei": "nonplugin",
	...
	"h1a3x/dsh-token-stats": "nonplugin",
	...
	"miyazawai/dsh-client-pricing": "nonplugin",
	...
	"poiuyjie/dsh-vision-opencode": "nonplugin",
	...
	"lhh010/dsh-input-history": "nonplugin",
	...
	"xaviermarquis93/dsh-plugin-conversation-outline": "nonplugin",
	...
	"qintsg/dsh-safe-delete": "nonplugin",
	...
	"auran-lu/dsh-client-ui-monitor": "nonplugin",
	"hxyz486/dsh-archived-conversations": "nonplugin",
	...
	"omdsh-dev/dsh-fun-ticker": "nonplugin",
	...
	"c-ling/dsh-plugin-pet": "nonplugin",
	...
	"wuwuzhige-sudo/dsh-terminal-panel": "nonplugin",
	...
	"jleon-account/dsh-client-usage": "nonplugin",
	...
	"hisaniwo/dsh-ergonomics": "nonplugin",
	...
	"misaki14987/dsh-theme-taffy": "nonplugin",
	"bujue600-arch/dsh-testgen": "plugin",
	"lco117/dsh-think-any-lang": "plugin",
	"kinyokun/dsh-session-import": "nonplugin",
	...
	"lordqyxz/dsh-ark-quota": "nonplugin",
	"jasonsun29/ds-balance-card": "nonplugin",
	...
	"bonexy226/dsh-cost-chip": "nonplugin",
	...
	"brucewu1126/dsh-web-background": "nonplugin",
	...
	"1690834643/dsh-usage-dashboard": "nonplugin",
...
function extractRegisteredNames(text, side) {
	...
	const re = side === "host" ? /(?:provide|service)\s*\(\s*['"]([^'"]{3,})['"]/g : /register\s*\(\s*['"]([^'"]{3,})['"]/g;
	let m;
	...
	return class ZatMarketGateway extends _classSuper {
				...
				execute: async (args) => {
					...
					for (let i = 0; i < picked.length; i += 3) {
						...
						const results = await Promise.all(batch.map((it, j) => {
							...
							if (kind !== "plugin" && kind !== "multi" && kind !== "unknown") return {
								status: "skip",
								summary: "不是可直接安装的插件,跳过体检",
								checks: []
							};
		...
		*/
		async detectKind(owner, repo) {
			...
			if (rootPkg.status === 200) try {
				return JSON.parse(rootPkg.body).dsh?.bundle?.patch ? "plugin" : "nonplugin";
			} catch {
				return "nonplugin";
			}
		...
		*/
		async analyzeCandidateConflicts(owner, repo, subdir) {
			...
			const meta = f.meta;
			if (f.missingEntries.length > 0) block.push(`入口文件缺失:${f.missingEntries.join("、")} — 构建产物没提交到仓库,装了也加载不起来`);
			for (const d of Object.keys(meta.dependencies || {})) if (d.startsWith("@deepseek-ai/")) block.push(`官方包${d}应为peer依赖`);
		...
		*/
		async analyzeCandidateHealth(owner, repo, kind) {
			...
			try {
				...
				if (kind === "unknown") this.kindCache.set((owner + "/" + repo).toLowerCase(), meta.dsh?.bundle?.patch ? "plugin" : "nonplugin");
				...
				if (missingEntries.length > 0) checks.push({
					level: "error",
				...
				if (officialDeps.length > 0) checks.push({
					level: "error",
					title: `官方包写进了 dependencies(共 ${officialDeps.length} 个)`,
		...
		*/
		async pnpmShell(command, dir, onProgress) {
			...
			if (IS_WIN) {
				...
				if (this.directDown) full = proxySetup + pnpmSetup + mirrorWin + run;
				else full = proxySetup + pnpmSetup + run + "; if ($LASTEXITCODE -ne 0) { $env:GIT_CONFIG_COUNT=1; $env:GIT_CONFIG_KEY_0='url.https://gh-proxy.com/https://github.com/.insteadOf'; $env:GIT_CONFIG_VALUE_0='https://github.com/';" + run + " }";
			} else full = this.directDown ? mirrorLin + command : command + " || { export GIT_CONFIG_COUNT=1; export GIT_CONFIG_KEY_0='url.https://gh-proxy.com/https://github.com/.insteadOf'; export GIT_CONFIG_VALUE_0='https://github.com/';" + command + " }";
		...
		*/
		...
		checkLlmUsable() {
			...
			try {
				...
				this.llmUsable = Array.isArray(providers) && providers.some((p) => p?.id === sel.provider);
			} catch {
				this.llmUsable = false;
			}
		...
		async translateBatch(batch) {
			...
			try {
				...
				let json = null;
				try {
					json = JSON.parse(out.replace(/```json|```/g, "").trim());
				} catch {
				...
				const result = {};
				if (json && typeof json === "object") for (let i = 0; i < batch.length; i++) {
		...
		async addSpec(owner, repo, subdir, taskId, preAnalysis) {
			...
			if (pnpmResult.outcome.exitCode !== 0) {
				...
				if (errText.includes("PREPARE_NOT_ALLOWED") || errText.includes("allowBuilds") || errText.includes("build script")) {
					...
					return {
						ok: false,
						packageName: null,
						message: "安装失败:插件要跑构建脚本被 pnpm 拦截(已还原)。在 pnpm-workspace.yaml 的 allowBuilds 里加上该插件名再试。"
					};
				}
				const lastLine = errText.trim().split(/\r?\n/).filter(Boolean).pop() || "";
				const reason = /UND_ERR|ECONN|ETIMEDOUT|Failed to connect|ENOTFOUND|network|fetch/i.test(errText) ? "连不上 GitHub/npm(网络问题)" : lastLine.slice(0, 120) || "未知原因";
				this.recordIssue("安装/更新失败", reason);
				return {
					ok: false,
					packageName: null,
					message: `安装失败:${reason}。已自动回滚,换个网络或稍后重试。`
				};
			}
			if (taskId) {
			...
			for (const name of deps) {
				...
				matched = true;
				try {
					if (JSON.parse(readFileSync(join(dir, "node_modules", name, "package.json"), "utf8")).dsh?.bundle?.patch) {
						bundles.push(name);
						added = name;
		...
		async list(page, sort, q, category) {
			try {
				...
				}).filter((item) => item.fullName.toLowerCase() !== SELF_REPO);
				const data = {
					ok: true,
					items,
		...
		async translate(items) {
			...
			for (const it of list) {
				...
				if (cached && Date.now() - cached.at < ZH_TTL) {
					map[it.fullName || key] = cached.zh;
					continue;
				}
				if (seen[key]) continue;
				seen[key] = true;
				pending.push({
					fullName: it.fullName || key,
					description: it.description || ""
				});
			}
				...
				const worker = async () => {
					while (next < batches.length) {
						...
						for (const fullName of Object.keys(res)) {
							...
							this.zhCache.set(fullName.toLowerCase(), {
								at: Date.now(),
								zh
							});
							map[fullName] = zh;
			...
			return {
				...
				map,
				llmUsable: this.checkLlmUsable(),
				pending: pending.length - Object.keys(map).length
			};
		}
		async installed() {
			try {
			...
			return {
				...
				taskId: this.launchTask(async (id) => {
					...
					if (r.outcome.exitCode !== 0) {
						...
						return {
							ok: false,
							message: `升级失败。手动一条命令搞定: dsh plugin --profile web add https://gh-proxy.com/https://github.com/${owner}/${repo}.git`
						};
					}
					this.invalidateListCache();
					await this.saveLastKnownGood();
			...
			try {
				...
				if (!s) {
					if ((await this.ghGet(`https://raw.githubusercontent.com/${o}/${r}/HEAD/package.json`)).status !== 200) {
								...
								return {
									...
									taskId: this.launchTask(async (id) => {
										...
										return res.ok ? {
											...
											message: `已安装 ${only.name || o + "/" + r} — 重启 dsh 生效${res.warning ? "。风险提示:" + res.warning : ""}`
										} : {
											ok: false,
											packageName: null,
											message: res.message
						...
						return {
							ok: false,
							message: "这个仓库不是可安装的 dsh 插件:里面没有找到插件声明(dsh.bundle)。它可能是一个技能包、工具库或代码仓库(只是打了 dsh-plugin 标签),无法通过市场一键安装。请到该仓库的 GitHub 页面查看它的使用方式。"
						};
					}
				}
				return {
					...
					taskId: this.launchTask(async (id) => {
						...
						return res.ok ? {
							ok: true,
							packageName: res.packageName,
							message: `已安装 github:${o}/${r}${s ? `#path:${s}` : ""} — 重启 dsh 后生效。${analysis.usage[0] || ""}${res.warning ? "。风险提示:" + res.warning : ""}`
						} : {
							ok: false,
							packageName: null,
							message: res.message
		...
		async deleteSession(sessionId) {
			try {
				...
				return {
					ok: true,
					message: `已删除会话 ${id}${warning ? "。" + warning : ""}`
				};
			} catch (err) {
				return {
					ok: false,
		...
		async installedList() {
			try {
				...
				for (const rec of new Set(Object.values(inst))) {
					...
					if (!owner || !repo) {
						if (rec.name.startsWith("@deepseek-ai/")) continue;
				...
				const worker = async () => {
					while (next < unique.length) {
						...
						if (repoRes.status === 200) try {
							enriched = JSON.parse(repoRes.body);
						} catch {
							enriched = null;
						}
						try {
		...
		async healthCheck() {
			...
			try {
				...
				for (const name of deps) try {
					...
					for (const d of Object.keys(meta.dependencies || {})) if (d.startsWith("@deepseek-ai/")) issues.push({
						level: "error",
						title: `${name} 把官方包 ${d} 写进了 dependencies`,
						detail: "官方包应使用 peerDependencies 引用;直接依赖会装出第二份拷贝并劫持官方 loader 行,可能让 dsh 起不来。建议反馈给插件作者。"
					});
					...
					if (!enabled && !name.startsWith("@deepseek-ai/")) issues.push({
						level: "info",
						title: `${name} 已停用`,
						detail: "已安装但不在启用名单。解决:点「一键修复」自动启用。",
						fixable: true
					});
				} catch {
					issues.push({
