...
// L2 业务服务层（ADR 0002）：自 main.js 迁出的壳无关模块。
const { DANGEROUS_EXT, fileRoots, isUnderFileRoots } = require('./lib/desktop/file-roots');
const procMod = require('./lib/desktop/proc');
const pathsMod = require('./lib/desktop/runtime-paths');
const profileMod = require('./lib/desktop/profile');
const guardBoxMod = require('./lib/desktop/guard-box');
const runtimePatchesMod = require('./lib/desktop/runtime-patches');
...
const previewMod = require('./lib/desktop/static-preview');
const {
  maintainShortcuts,
...
const { startJunctionWatchdog } = junctionPatrolMod;
const { runClientUpdateFlow, offerPendingClientUpdate } = clientUpdateMod;
const { getPreviewStaticPort, startPreviewStaticServer, verifyBundledModules } = previewMod;
shortcutsMod.init({
  log,
...
clientUpdateMod.init({
  ...
  getUserDataDir: () => userDataDir,
  getDshHome: () => dshHome,
  showUpdateWindow: (version, kind) => showUpdateWindow(version, kind),
  makeUpdateProgressPusher: (win) => makeUpdateProgressPusher(win),
  // 客户端更新交接前的退出序列（原 runClientUpdateFlow / offerPendingClientUpdate 内联逻辑）：
  // 置退出标记 → 落 cleanExit → 中止 updater/会话监听 → 等待 dsh 进程树真正退出。
...
// （实现已迁至 lib/desktop/guard-box.js，ADR 0002）
// ---------------------------------------------------------------------------
...
function registerChromeIpc() {
  ipcMain.handle('chrome:init', async (event) => {
    ...
    return {
      ...
      iconDataUri,
      repoUrls: urls,
      staticPort: getPreviewStaticPort(),
    };
  });
  ...
  app.on('before-quit', (event) => {
    ...
    (async () => {
      try {
        ...
        const marketOpChild = marketMod.getMarketOpChild();
