...
// 契约对象原为 preload.js（随 Electron 壳退役），现为 Tauri 侧同源实现
// tauri-shell/sidecar/bridge.ts（注入 chrome 的单一实现）。
...
test('bridge declares its titlebar height on <html> for client plugins', () => {
  // BAR_HEIGHT 与声明的属性值必须同源（同一常量插值），不能硬编码两份
  const barHeight = Number(bridge.match(/(?:const|var|let) BAR_HEIGHT = (\d+)/)?.[1]);
  assert.ok(barHeight > 0, 'BAR_HEIGHT must be defined');
  assert.match(
    bridge,
    /setAttribute\('data-dsh-title-bar-height', String\(BAR_HEIGHT\)\)/,
