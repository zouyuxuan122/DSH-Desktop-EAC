/**
 * session-encoding-heal.ts — 会话编码一致性自愈（启动失败救援）（Task 7.1
 * 自 session-encoding-heal.js 迁 TS）。
 *
...
export function isEncodingMismatch(errText: unknown): boolean {
...
const LIVE_SUFFIX: Record<'zstd' | 'none', string> = { zstd: '.jsonl.zstd', none: '.jsonl' };
const STALE_SUFFIX: Record<'zstd' | 'none', string> = { zstd: '.jsonl', none: '.jsonl.zstd' };

/** healSessionEncodingConflicts 的选项。 */
export interface HealEncodingOpts {
  /** 后端压缩编码（默认 zstd，与 dsh-session-persistence-jsonl 默认一致）。 */
 ...
 * `<name>.bak-<时间戳>`，绝不删除。
 *
 * @returns 已归档（改名后）的文件绝对路径
 */
export function healSessionEncodingConflicts(sessionsDir: string, opts: HealEncodingOpts = {}): string[] {
  const compression: 'zstd' | 'none' = opts.compression === 'none' ? 'none' : 'zstd';
  ...
  for (const project of projects) {
    ...
    try {
      sessionDirs = fs.readdirSync(projDir, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const sess of sessionDirs) {
      ...
      try {
        ...
        log('session-heal', `归档 ${stale} 失败: ` + String((err as Error).message || err));
      }
    }
  }
  return archived;
