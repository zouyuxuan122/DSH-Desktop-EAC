...
const cu = require('../client-updater');
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      let got = 0;
      let first2 = '';
      const req = net.request({ url: setup.url, redirect: 'follow' });
      const timer = setTimeout(() => req.destroy(new Error('下载探测超时')), 30000);
      req.on('response', (res) => {
        clearTimeout(timer);
        res.on('data', (c) => {
          if (got === 0) first2 = c.slice(0, 2).toString('latin1');
          got += c.length;
          if (got >= 65536) { try { req.destroy(); } catch {} }
        });
        res.on('end', () => {
          if (res.statusCode !== 200) return reject(new Error('下载探测 HTTP ' + res.statusCode));
          if (first2 !== 'MZ') return reject(new Error('下载探测内容非 PE 文件（头 ' + JSON.stringify(first2) + '）'));
          console.log('PROBE-DOWNLOAD status=' + res.statusCode + ' bytes>=' + got + ' MZ=OK');
          resolve();
        });
        res.on('error', (err) => {
          if (got >= 65536 && first2 === 'MZ') { console.log('PROBE-DOWNLOAD bytes>=' + got + ' MZ=OK（提前截断）'); return resolve(); }
          reject(err);
        });
      });
      req.on('error', (err) => {
        if (got >= 65536 && first2 === 'MZ') return resolve();
        reject(err);
      });
      req.end();
    });
