...
const FULL: EditionInfo = {
  ...
  tag: "v5.3.6",
  releaseName: "v5.3.6",
  publishedAt: "2026-09-03T10:54:14Z",
  bodyExcerpt: "v5.1.0",
  asset: { name: "Deepseek-Harness-EAC-5.3.6-portable.zip", size: 239487331, url: "" },
  shaUrl: null,
};
...
const handlers: Record<string, CmdHandler> = {
  ...
  // 安全中心 / 版本管理 / 导入
  list_editions: () => [FULL, LITE],
  check_updates: () => demoConfig.instances,
  probe_import: () => ({ ok: false, reason: "演示模式不可用", dir: "", exe: null, edition: "unknown", version: "", suggestedName: "", dshHomeExists: false }),
  import_instance: () => demoConfig.instances[0],
  unregister_instance: () => null,
  upgrade_instance: () => null,
  rollback_instance: () => "已回退（演示）",
  get_doctor: () => [
    { id: "exe", title: "主程序", level: "ok", detail: "D:\\demo\\dsh-eac-shell.exe", fix: null },
    { id: "profile", title: "Profile 基础 bundles", level: "ok", detail: "3 项 bundles 就绪", fix: null },
    { id: "native_fs_ext", title: "fs-ext 原生模块", level: "warn", detail: "上游便携包未携带 fs_ext.node，可一键修补", fix: "native_fs_ext" },
  ],
  doctor_fix: () => "已修复（演示）",
  plugin_snapshots: () => [
    { ts: Date.now() - 3600_000, reason: "安装 @dsh-plugin/dsh-loader 前自动快照", deps: 2 },
    { ts: Date.now() - 7200_000, reason: "停用 dsh-better-sidebar 前自动快照", deps: 1 },
  ],
  plugin_restore_snapshot: () => "snap-demo",
  quarantine_all: () => 0,
  quarantine_restore: () => null,
  quarantine_purge: () => null,
  reinstall_deps: () => "deps-demo",
